...
fn main() {
    let formats = vec![
        "gf12", "f32", "bf16", "gf16", "gf8", "posit8", "mxfp8", "nf4", "int8",
    ];
    ...
    let train_data =
        env::var("TRIOS_TRAIN_DATA").unwrap_or_else(|_| "/work/data/tiny_shakespeare.txt".into());
    let val_data =
        env::var("TRIOS_VAL_DATA").unwrap_or_else(|_| "/work/data/tiny_shakespeare_val.txt".into());
