...
use crate::objective::{NcaObjective, nca_entropy_loss};
...
pub fn run_single(args: &TrainArgs) -> Result<RunOutcome> {
    ...
    for step in 1..=args.steps {
        ...
        if step % args.eval_every == 0 || step == args.steps {
            ...
            println!("seed={} step={} val_bpb={:.4} ema_bpb={:.4} best={:.4} nca_h={:.3} t={:.1}s",
                args.seed, step, vbpb, ema_bpb, best_bpb, last_nca_entropy, t0.elapsed().as_secs_f64());
        }
    }
...
pub fn run_single_muon(args: &TrainArgs, use_cwd: bool) -> Result<RunOutcome> {
    ...
    eprintln!("=== P1 {} seed={} steps={} hidden={} ===", label, args.seed, args.steps, args.hidden);
    let train = load_data(&args.train_path);
    let val = load_data(&args.val_path);
    ...
    for step in 1..=args.steps {
        ...
        if step % args.eval_every == 0 || step == args.steps {
            ...
            if ema_bpb < best_bpb && ema_bpb.is_finite() { best_bpb = ema_bpb; }
            println!("{} seed={} step={} val_bpb={:.4} ema_bpb={:.4} best={:.4} nca_h={:.3} t={:.1}s",
                label, args.seed, step, vbpb, ema_bpb, best_bpb, last_nca_entropy, t0.elapsed().as_secs_f64());
        }
    }
