...
The new fleet keeps the fresh numeric range (jumping past the previous
...
```bash
# Seed 100
railway add --service igla-trainer-seed-100 --variables "TRIOS_SEED=100"
railway up --service igla-trainer-seed-100

# Seed 101
railway add --service igla-trainer-seed-101 --variables "TRIOS_SEED=101"
railway up --service igla-trainer-seed-101

# Seed 102
railway add --service igla-trainer-seed-102 --variables "TRIOS_SEED=102"
railway up --service igla-trainer-seed-102
```
...
# Show commands without executing (default: 100 101 102)
...
Or check logs:
```bash
railway logs --service igla-trainer-seed-100
railway logs --service igla-trainer-seed-101
railway logs --service igla-trainer-seed-102
```
...
- **TRIOS_SEED**: DYNAMIC — set per-service via Railway `--variables` at deploy
  time. New fleet uses 100/101/102.
- Default seed in `Dockerfile`: `100` (overridden by Railway env var).
...
```dockerfile
ENV TRIOS_SEED=100  # Default for the new fleet, overridden by Railway env var
