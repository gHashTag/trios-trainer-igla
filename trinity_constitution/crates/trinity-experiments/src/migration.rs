//! Migration 0001 — INITIAL (immutable, ONE TIME ONLY)
//!
//! # Constitutional mandate (Law 2)
//!
//! This is the **ONLY** migration. After 0001, no ALTER TABLE.
//! New fields → new JSONB keys or new tables.

/// Migration 0001 SQL — initial schema
...
mod tests {
    ...
    #[test]
    fn migration_is_valid_sql() {
        // Basic sanity check — ensure no syntax errors
        assert!(!MIGRATION_0001.is_empty());
        assert!(MIGRATION_0001.contains("CREATE TABLE IF NOT EXISTS strategy_experiments"));
        assert!(MIGRATION_0001.contains("status TEXT NOT NULL CHECK"));
        assert!(MIGRATION_0001.contains("bpb_curve JSONB"));
    }

    #[test]
    fn migration_has_required_invariants() {
        assert!(MIGRATION_0001.contains("CHECK (status NOT IN ('done','failed')"));
        assert!(MIGRATION_0001.contains("CHECK (final_bpb IS NULL OR (final_bpb > 0"));
    }

    #[test]
    fn migration_has_indexes() {
        assert!(MIGRATION_0001.contains("CREATE INDEX IF NOT EXISTS idx_se_pending"));
        assert!(MIGRATION_0001.contains("WHERE status = 'pending'"));
    }
}
