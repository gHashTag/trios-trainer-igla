...
- F2 §3.5.4 says "714 passing tests" (src/lib.rs only); #1021 §1.3
  says "849 tests" (total = lib + per-binary + integration). Different
  scopes — gated for self-consistency.
- F2 §1 abstract says "849 tests"; #1021 §1.3 says "849 tests" — same
...
SCOPED_DIFF_CLAIMS: list[tuple[str, Path, str, int, int, str]] = [
    ...
    # F2 §8.2 "849 tests grouped by source: 735 in src/lib.rs"
    (
...
RELATIONAL_CLAIMS: list[tuple[str, Path, str, Path, str, str]] = [
    ...
    (
        ...
        ISSUE1021_PAPER,
        r"849 tests across `src/lib\.rs`, (\d+) F2 binaries",
        F2_PAPER,
        "ge",
