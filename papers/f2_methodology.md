...
The framework is open-source in Rust with 10 binaries, 809
...
`papers/scripts/generate_appendix_d.sh`) lists 809 tests grouped by
source: 714 in `src/lib.rs`, the remainder distributed across
...
ten binaries sharing a stable CSV contract, and 809 tests including
...
cargo test --lib         # exits 0 with 714 passing tests (§8.2)
```
...
The full test inventory (809 tests across `src/lib.rs`, 10 F2 binaries,
...
- **`papers/scripts/run_all_checks.sh`** (~60 s warm; **15–30 min
  cold**) — single-shot CI gate. Currently chains **34 stages**
  ...
  not a stage of itself); the **other twenty-seven stages are gates
  ...
  matching commit on HEAD), and the dependency-graph verifier
  ...
  the additional 27 are documented in the follow-up paper's
